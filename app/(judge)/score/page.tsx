...
export default function ScorePage() {
  ...
  useEffect(() => {
    async function loadLanesAndWorkouts() {
      ...
      // A heat generated by the workout builder is tied to exactly one
      // workout (heats.workout_id, migration-022) — always derive the
      // selection from that FK rather than defaulting to the first
      // workout in the division, which silently mismatched whenever a
      // division had more than one workout and the heat wasn't for the
      // first one. Only legacy heats predating the workout builder
      // (workout_id null) fall back to a pickable list.
      setSelectedWorkoutId(heat?.workoutId ?? mappedWorkouts[0]?.id ?? "");
    }
        ...
        <>
          ...
          {selectedHeatId && (
            <>
              ...
              {selectedHeat?.workoutId ? (
                // This heat is tied to a specific workout (migration-022)
                // — show it as a fixed label, not a picker, so it can
                // never be set to a different workout than the one this
                // heat's scores actually belong to.
                <div className="bg-white border border-ink/10 rounded-lg px-4 py-3 text-sm">
                  <span className="text-xs uppercase tracking-wider text-ink/50 block">Workout</span>
                  <span className="font-semibold">{selectedWorkout?.name ?? selectedHeat.workoutName}</span>
                </div>
              ) : workouts.length > 0 ? (
                <select
